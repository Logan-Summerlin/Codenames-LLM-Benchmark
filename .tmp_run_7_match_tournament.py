...
OUT_DIR = ROOT / "runs" / f"limited-random-14-models-7way-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}"
    ...
    pairs.append(
        {
            ...
            "transcript": str(OUT_DIR / f"match-{i // 2 + 1:02d}-transcript.json"),
        }
    )
