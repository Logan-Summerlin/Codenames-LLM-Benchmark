#!/usr/bin/env python3
"""Run a live OpenRouter double round robin among the four strongest Codenames models.

This helper is intentionally narrow: it selects the four top models, pins
provider order / reasoning effort per model, and executes two games at a time
so the benchmark can progress in parallel without overloading the local host.
"""
...
from codenames_benchmark.tournament import ModelSpec, OPENROUTER_CODENAMES_MODELS, schedule_double_round_robin
...
def default_output_dir() -> Path:
    ...
    return ROOT / "runs" / f"top4-double-round-robin-{stamp}"
...
def main() -> int:
    ...
    parser.add_argument("--seed-prefix", default="top4-double-round-robin")
    ...
    schedule = schedule_double_round_robin([m.slug for m in models], seed_prefix=args.seed_prefix)
    ...
    write_json(
        output_dir / "manifest.json",
        {
            ...
            "benchmark": "codenames-openrouter-top4-double-round-robin",
